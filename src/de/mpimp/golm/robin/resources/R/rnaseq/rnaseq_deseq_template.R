...
MIN_LFC2        	<- ifelse(__MIN_LOGFOLDCHANGE_2_WANTED__, 1, 0)
...
if (length(replicated) < 1) {
	...
	cds <- estimateDispersions( cds, method="__DISPERSION__" )
} else {
	test <- try( estimateDispersions( cds, method="__DISPERSION__", fitType="parametric" )) # there are more options availbale - include in next update
