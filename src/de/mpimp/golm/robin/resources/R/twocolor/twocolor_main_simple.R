## template for limma-based analysis of simple
## two group replicate and dye swap two-color
...
for (i in 1:ncol(fit2)) {
	...
	if (length(colnames(fit2)[i]) == 0) {
		header <- c(header, colnames(fit2$cov.c)[i], paste("d_",colnames(fit2$cov.c)[i], sep=""))
	} else {	
