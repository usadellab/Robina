...
#### Modified from: 
#### R script to
#### 	Produce Venn Diagrams with 1 to 4 groups
...
robinVennDiagram  <- function (object, include = "both", names, mar = rep(1, 4), cex = 1.0, 
    lwd = 2, counts.col, show.include, ...) 
{
	circle.col = c(2:(2+ncol(object)))
	# set up a layout for a fake plot that will take up the legend
	layout(matrix(1:2, nc = 2), c(3,2))
...
#	rect(-3.5, -3.5, 3.5, 3.5)
	...
	legend("topleft", legend=names, col=circle.col, pch=15, bty="n", cex=0.75)
